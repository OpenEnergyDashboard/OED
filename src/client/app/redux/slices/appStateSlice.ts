...
export const appStateSlice = createThunkSlice({
	...
	reducers: create => ({
		...
		setEnableAllUnits: create.reducer<boolean>((state, action) => {
			state.enableAllUnits = action.payload;
		}),
	...
	selectors: {
		...
		selectEnableAllUnits: state => state.enableAllUnits
	}
});
