...
export default function MapChartComponent() {
	const { meterArgs, groupArgs, meterShouldSkip, groupShouldSkip } =
		useAppSelector(selectMapChartQueryArgs);
	const { data: meterReadings, isLoading: meterIsFetching } =
		readingsApi.useBarQuery(meterArgs, { skip: meterShouldSkip });
	const { data: groupData, isLoading: groupIsFetching } =
		readingsApi.useBarQuery(groupArgs, { skip: groupShouldSkip });
	...
	if (selectedMap !== 0) {
		...
		if (map && map.origin && map.opposite) {
			...
			const imageDimensionNormalized =
				normalizeImageDimensions(imageDimensions);
			...
			const scaleOfMap = calculateScaleFromEndpoints(
				origin,
				opposite,
				imageDimensionNormalized,
				map.northAngle
			);
			...
			if (unitID !== -99) {
				...
				if (selectUnitState !== undefined) {
					...
					if (selectUnitState.unitRepresent === UnitRepresentType.quantity) {
						...
						unitLabel =
							selectUnitState.identifier + " * time / day ≡ quantity / day";
					}
			...
			for (const meterID of selectedMeters) {
				...
				if (gps !== undefined && gps !== null && meterReadings !== undefined) {
					...
					if (
						!areaNormalization ||
						(meterArea > 0 &&
							meterDataById[meterID].areaUnit != AreaUnitType.none)
					) {
						if (areaNormalization) {
							// convert the meter area into the proper unit, if needed
							meterArea *= getAreaUnitConversion(
								meterDataById[meterID].areaUnit,
								selectedAreaUnit
							);
						}
						...
						const meterGPSInUserGrid: CartesianPoint = gpsToUserGrid(
							imageDimensionNormalized,
							gps,
							origin,
							scaleOfMap,
						...
						if (
							itemMapInfoOk(meterID, DataType.Meter, map, gps) &&
							...
							if (readingsData !== undefined && !meterIsFetching) {
								...
								// The hover text.
								hoverText.push(
									`<b> ${timeReading} </b> <br> ${label}: ${averagedReading.toPrecision(
										6
									)} ${unitLabel}`
								);
							}
			...
			for (const groupID of selectedGroups) {
				...
				if (gps !== undefined && gps !== null && groupData !== undefined) {
					...
					if (
						!areaNormalization ||
						(groupArea > 0 &&
							groupDataById[groupID].areaUnit != AreaUnitType.none)
					) {
						if (areaNormalization) {
							// convert the meter area into the proper unit, if needed
							groupArea *= getAreaUnitConversion(
								groupDataById[groupID].areaUnit,
								selectedAreaUnit
							);
						}
						// Convert the gps value to the equivalent Plotly grid coordinates on user map.
						...
						// To start, calculate the user grid coordinates (Plotly) from the GPS value. This involves calculating
						// it coordinates on the true north map and then rotating/shifting to the user map.
						const groupGPSInUserGrid: CartesianPoint = gpsToUserGrid(
							imageDimensionNormalized,
							gps,
							origin,
							scaleOfMap,
						...
						// Only display items within valid info and within map.
						if (
							itemMapInfoOk(groupID, DataType.Group, map, gps) &&
			...
			const minDimension = Math.min(
				imageDimensionNormalized.width,
				imageDimensionNormalized.height
			);
			...
			const maxCircleSize =
				Math.PI * Math.pow((minDimension * maxFeatureFraction) / 2, 2);
			...
			minFeatureFractionOfMax =
				minFeatureFractionOfMax * maxCircleSize < circleSizeThreshold
					? circleSizeThreshold / maxCircleSize
					: minFeatureFractionOfMax;
			...
			if (range === 0) {
				...
				showInfoNotification(
					"All values are negative so the circle sizes act as if value range was positive which may change the relative sizes."
				);
				...
				if (Math.abs(max) < minValue) {
					...
					// TODO Must be internationalized.
					showInfoNotification(
						"Some values are close to zero so the small circle sizes may be a little larger to be visible"
					);
					shift = min - minValue;
				} else {
					shift = min + max;
				...
				if (max > 0 && min < 0) {
					...
					// TODO Must be internationalized.
					showInfoNotification(
						"There are negative and positive values and this impacts relative circle size."
					);
				}
			}
	...
	return <Plot data={data as Plotly.Data[]} layout={layout} />;
}
