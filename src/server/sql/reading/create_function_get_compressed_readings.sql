...
CREATE OR REPLACE FUNCTION compressed_readings(
    meter_ids INTEGER[],
		-- All readings in this timestamp from the above meters will be compressed.
		...
		WITH readings_by_compression_point AS (
			SELECT
				...
				*/
				...
				EXTRACT(EPOCH FROM least(r.end_timestamp, compressed.start + point_width)
													 - greatest(r.start_timestamp, compressed.start)) AS duration
			FROM readings r
