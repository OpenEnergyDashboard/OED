...
router.get('/token', optionalAuthMiddleware, async (req, res) => {
	...
	if (!validate(token, validParams).valid) {
		failure(res, HTTP_CODES.FORBIDDEN, null, 'No token provided or JSON was invalid.');
	} else if (token) {
		jwt.verify(token, secretToken, async (err, decoded) => {
			if (err) {
				failure(res, HTTP_CODES.UNAUTHORIZED, err, 'Failed to authenticate token.', LogLevel.SILENT);
			} else {
				try {
					const conn = getConnection();
					...
					failure(res, HTTP_CODES.UNAUTHORIZED, error, 'User does not exist in database.', LogLevel.SILENT);
				}
			}
		});
	} else {
		failure(res, HTTP_CODES.FORBIDDEN, null, 'No token provided.');
	}
});
...
router.post('/create', adminAuthMiddleware('create a user.'), async (req, res) => {
	...
	if (!validatorResult.valid) {
		...
		try {
			...
			if (currentUser !== null) {
				failure(res, HTTP_CODES.BAD_REQUEST, null, `user ${username} already exists so cannot create`);
			} else {
	...
	if (!validatorResult.valid) {
		...
		try {
			...
			if (user.username === username) {// Admins cannot delete themselves
				failure(res, HTTP_CODES.BAD_REQUEST, null, 'Admins cannot delete themselves.');
			} else {
