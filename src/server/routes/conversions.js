...
router.get(
	...
	async (res, req) => {
		...
		if (!validate(req.params, validParams).valid) {
			...
			try {
				const rows = await Conversion.getBySourceDestination(
					req.params.sourceId,
				...
				log.error(
					`Error while performing GET using source and destination id: ${err}`,
					err
				);
				res.sendStatus(500);
			}
...
router.post(
	"/create",
	adminAuthMiddleware("create a conversion"),
	async (req, res) => {
		const validParams = {
			type: "object",
			require: [
...
router.post(
	"/delete",
	adminAuthMiddleware("delete a conversion"),
	async (req, res) => {
		const validParams = {
			type: "object",
			required: ["sourceId", "destinationId"],
