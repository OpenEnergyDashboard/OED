...
class Meter {
	...
	constructor(id, name, url, enabled, displayable, type, meterTimezone, gps = undefined, identifier = name, note, area,
		...
		previousEnd = moment(0).utc(), unitId = -99, defaultGraphicUnit = -99, readingFrequency = process.env.OED_SITE_READING_RATE) {
	...
	static mapRow(row) {
		var meter = new Meter(row.id, row.name, row.url, row.enabled, row.displayable, row.meter_type, row.default_timezone_meter,
			...
			row.end_only_time, row.reading, row.start_timestamp, row.end_timestamp, row.previous_end, row.unit_id,
			row.default_graphic_unit, row.reading_frequency);
	...
	merge(name = this.name, url = this.url, enabled = this.enabled, displayable = this.displayable, type = this.type,
		...
		previousEnd = this.previousEnd, unitId = this.unitId, defaultGraphicUnit = this.default_graphic_unit,
