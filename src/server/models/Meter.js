...
class Meter {
	 ...
	 * @param meterTimezonep Default timezone for meter
	 */
	constructor(id, name, ipAddress, enabled, displayable, type, meterTimezone) {
		this.id = id;
		this.name = name;
	...
	static mapRow(row) {
		return new Meter(row.id, row.name, row.ipaddress, row.enabled, row.displayable, row.meter_type, row.default_timezone_meter);
	}
