...
async function loadArrayInput(dataRows, ipAddress, mapRowToModel, isCummulative, conditionSet, conn) {
	try {
		readingsArray = mapRowToModel(dataRows);
		if (isCummulative) {
			readingsArray = handleCummulative(readingsArray);
		}
		readings = convertToReadings(readingsArray, ipAddress, conditionSet);
