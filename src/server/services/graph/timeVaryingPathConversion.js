	...
	for (let i = 0; i < path.length - 1; ++i) {
		...
		for (let segmentIndex = 0; segmentIndex < segments.length; segmentIndex++) {
			...
			if (curSegment.weekPatternsId == null) {
				...
				console.log("DEBUG: curSegment:", curSegment);
				...
				console.log("DEBUG: ruleInfo:", ruleInfo);
				...
				ruleInfo.forEach((info) => {
					occurrences.push(info.rule.between(datetime(2024, 1, 1), datetime(2024, 1, 15)));

					// 2.b. If reversed is true then invert the slope/intercept for each conversion using invertConversion().
					if (reversed) {	// CHANGE!!!!
						// Reversed so invert segment found.
						const { convertedSlope, convertedIntercept } = invertConversion(info.slope, info.intercept);
