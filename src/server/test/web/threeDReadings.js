	...
	mocha.describe('readings test, test if data returned by API is as expected', () => {
		mocha.describe('for threeD graphs', () => {
			mocha.describe('for meters', () => {
				...
				mocha.it(`4 hour readings, 3 point/day returns 4, full time range and quantity units of kWh as kWh`, async () => {
					// Create 2D array for meter to feed into the database
					// Note the meter ID is set so we know what to expect when a query is made.
					const meterData = [
						{
				...
				mocha.it('15 minute readings, 8 readings/day with holes, extended time range and quantity units of kWh as kWh', async () => {
					// Create 2D array for meter to feed into the database
					// Note the meter ID is set so we know what to expect when a query is made.
					const meterData = [
						{
					...
					const res = await chai.request(app).get(`/api/unitReadings/threeD/meters/${METER_ID}`)
						.query({
							...
							graphicUnitId: unitId, readingInterval: 8
						});
					expectThreeDReadingToEqualExpected(res, expected, 8);
				});
			});
