 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. 
	...
	Directions for creating reading tests (not needed for rejection tests)
		...
		5) write your test using expectReadingToEqualExpected as necessary
*/
...
// Test groups as well
// Optimize testing to run faster, each test is taking ~4 seconds on M1 Macbook which is way too slow
// Comments
		...
		mocha.describe('for line charts', () => {
			mocha.describe('for meters', () => {
				...
				mocha.it('should have the expected readings for 15 minute reading intervals and quantity units', async () => {
					...
					await prepareTest(unitData, conversionData, meterData);
					let expected = await parseExpectedCsv('src/server/test/web/readingsData/expected_ri_15_mu_kWh_gu_kWh_st_-inf_et_inf.csv');
					const res = await chai.request(app).get(`/api/unitReadings/line/meters/${METER_ID}`)
						.query({ timeInterval: ETERNITY.toString(), graphicUnitId: 1 });
...
/* 				mocha.it('should have the expected readings for 15 minute reading intervals and flow units', async () => {
					...
					await prepareTest(unitData, conversionData, meterData);
					let expected = await parseExpectedCsv();
			...
			mocha.describe('for groups', () => {
				...
				mocha.it('response should have valid reading and timestamps,', async () => {
					...
					const conversionData = [['Electric_Utility', 'kWh', false, 1, 0, 'Electric_Utility → kWh']];
					const meterData = [
						['Electric Utility kWh', 'Electric_Utility', 'kWh', true, undefined,'special meter', 'test/web/readingsData/readings_ri_15_days_75.csv', false, METER_ID],
						['Electric Utility kWh 2-6', 'Electric_Utility', 'kWh', true, undefined, 'special meter', 'test/web/readingsData/readings_ri_15_days_75.csv', false, (METER_ID + 1)]
					];
