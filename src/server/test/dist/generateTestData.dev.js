...
var test1 = sin(0) == Math.sin(0) && sin(Math.PI * 2) == Math.sin(Math.PI * 2) && sin(Math.PI) == Math.sin(Math.PI);
console.log("Test 1: We should have a proper sin function: ".concat(test1));
/* Test 2: Sine Wave
 * We have a function that takes an array of numbers(x-values) and returns the value of the 
 * Sine wave on that numbers
 */

var test2 = arraysMatch(sineWave([0]), [0].map(Math.sin)) && arraysMatch(sineWave([0, TWO_PI]), [0, TWO_PI].map(Math.sin));
console.log("Test 2: We should have a proper sine wave function: ".concat(test2));
/* Functions
 */
