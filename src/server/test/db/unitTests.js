...
mocha.describe('Units', () => {
	mocha.it('can be saved and retrieved', async () => {
		...
		const unitTypePreInsert = new Unit(undefined, 'Unit', 'Index', Unit.unitRepresentType.UNUSED, 
											1000, Unit.unitType.UNIT, 0, 'Suffix', Unit.displayableType.ALL, true, '');
	...
	mocha.it('can be saved, edited, and retrieved', async () => {
		const conn = testDB.getConnection();
		const unitPreInsert = new Unit(undefined, 'Unit', 'Index', Unit.unitRepresentType.UNUSED, 
										1000, Unit.unitType.UNIT, 0, '', Unit.displayableType.ALL, true, '');
	...
	mocha.it('can get visible unit of type meter', async () => {
		const conn = testDB.getConnection();
		const unitTypeUnit = new Unit(undefined, 'Unit', 'Unit', Unit.unitRepresentType.UNUSED, 
										3600, Unit.unitType.UNIT, 0, '', Unit.displayableType.ALL, true, '');
		const unitTypeMeterAll = new Unit(undefined, 'Meter All', 'Meter All', Unit.unitRepresentType.UNUSED, 
	...
	mocha.it('can get visible unit of type unit or suffix', async () => {
		const conn = testDB.getConnection();
		const unitTypeMeter = new Unit(undefined, 'Meter', 'Meter', Unit.unitRepresentType.UNUSED, 
										3600, Unit.unitType.METER, 0, '', Unit.displayableType.ALL, true, '');
		const unitTypeUnitAll = new Unit(undefined, 'Unit All', 'Unit All', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.UNIT, 1, '', Unit.displayableType.ALL, true, '');
		const unitTypeUnitAdmin = new Unit(undefined, 'Unit Admin', 'Unit Admin', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.UNIT, 2, '', Unit.displayableType.ADMIN, true, '');
		const unitTypeSuffixAll = new Unit(undefined, 'Suffix All', 'Suffix All', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.SUFFIX, 3, '', Unit.displayableType.ALL, true, '');
		const unitTypeSuffixNone = new Unit(undefined, 'Suffix None', 'Suffix None', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.SUFFIX, 4, '', Unit.displayableType.NONE, true, '');
	...
	mocha.it('should only get units of type meter', async () => {
		...
		const unitTypeMeterAll = new Unit(undefined, 'Meter All', 'Meter All', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.METER, 1, '', Unit.displayableType.ALL, true, '');
		const unitTypeMeterNone = new Unit(undefined, 'Meter None', 'Meter None', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.METER, 2, '', Unit.displayableType.NONE, true, '');
		await unitTypeUnit.insert(conn);
		await unitTypeMeterAll.insert(conn);
	...
	mocha.it('should only get units of type unit', async () => {
		...
		const unitTypeUnitAll = new Unit(undefined, 'Unit All', 'Unit All', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.UNIT, 1, '', Unit.displayableType.ALL, true, '');
		const unitTypeUnitNone = new Unit(undefined, 'Unit None', 'Unit None', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.UNIT, 2, '', Unit.displayableType.NONE, true, '');
		await unitTypeMeter.insert(conn);
		await unitTypeUnitAll.insert(conn);
	...
	mocha.it('should only get units with suffix', async () => {
		const unitTypeUnit = new Unit(undefined, 'Unit', 'Unit', Unit.unitRepresentType.UNUSED, 
										3600, Unit.unitType.UNIT, 0, '', Unit.displayableType.ADMIN, true, '');
		const unitTypeSuffixAll = new Unit(undefined, 'Suffix All', 'Suffix All', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.SUFFIX, 1, 'Suffix 1', Unit.displayableType.ALL, true, '');
		const unitTypeSuffixNone = new Unit(undefined, 'Suffix None', 'Suffix None', Unit.unitRepresentType.UNUSED, 
											3600, Unit.unitType.SUFFIX, 2, 'Suffix 2', Unit.displayableType.NONE, true, '');
		await unitTypeUnit.insert(conn);
		await unitTypeSuffixAll.insert(conn);
		await unitTypeSuffixNone.insert(conn);
