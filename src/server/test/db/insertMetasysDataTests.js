...
mocha.describe('PIPELINE: insert MetasysData from csv file', () => {
	mocha.describe('with metasys-duplicate meter', () => {
		mocha.beforeEach(async () => {
			...
			await new Meter(undefined, 'metasys-duplicate', null, false, true, Meter.type.METASYS, null, undefined).insert(conn);
		});
	...
	mocha.describe('with metasys-invalid meter', () => {
		mocha.beforeEach(async () => {
			conn = testDB.getConnection();
			await new Meter(undefined, 'metasys-invalid', null, false, true, Meter.type.METASYS, null, undefined).insert(conn);
		});
	...
	mocha.describe('with metasys-valid meter', () => {
		mocha.beforeEach(async () => {
			conn = testDB.getConnection();
			await new Meter(undefined, 'metasys-valid', null, false, true, Meter.type.METASYS, null, undefined).insert(conn);
		});
