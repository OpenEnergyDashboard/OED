...
mocha.describe('Load data from csv file', () => {
	...
	mocha.it('as array', async () => {
		...
		await loadCsvInput(testFilePath, arrayMeter.id, mapRowsToModel, false, false, undefined, conn);
	...
	mocha.it('as stream', async () => {
		...
		await loadCsvInput(testFilePath, streamMeter.id, mapRowsToModel, true, false, undefined, conn);
