...
mocha.describe('PIPELINE: Load data from csv file', () => {
	...
	mocha.it('as array', async () => {
		...
		await loadCsvInput(testFilePath, arrayMeter.id, mapRowsToModel, false, false, 1, undefined, conn);
	...
	mocha.it('as stream', async () => {
		...
		await loadCsvInput(testFilePath, streamMeter.id, mapRowsToModel, true, false, 1, undefined, conn);
