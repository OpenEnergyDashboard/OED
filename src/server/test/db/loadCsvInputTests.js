...
mocha.describe('PIPELINE: Load data from csv file', () => {
	...
	mocha.it('as array', async () => {
		...
		const arrayMeter = new Meter(undefined, 'test_array', 1, true, true, Meter.type.MAMAC, null, unknown);
	...
	mocha.it('as stream', async () => {
		const conn = testDB.getConnection();
		const arrayInput = await readCsv(testFilePath);
		const streamMeter = new Meter(undefined, 'test_stream', 2, true, true, Meter.type.MAMAC, null, unknown);
